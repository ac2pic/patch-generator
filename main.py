...
def fixPatches(elfFh, elfName, patchSetSet):
    prot = patchSet["prot"]
    segment = ""
    if prot == 0b101:
        segment = ".text"
    ...
    for idx, patchEntry in enumerate(patchSet["patches"]):
        ...
        patch = bytes.fromhex(patch.strip())
...
json.dump(patch_data,open("out.json", "w"))
